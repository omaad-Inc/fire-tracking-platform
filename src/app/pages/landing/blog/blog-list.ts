                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    @for (post of filteredPosts(); track post.slug) {
                        <a [routerLink]="['/', lang, 'blog', post.slug]"
                           class="group flex flex-col rounded-2xl overflow-hidden bg-surface-0 border border-surface-200 hover:border-brand-300 hover:shadow-lg hover:shadow-brand-100/40 transition-all cursor-pointer">
                            ...
                            <div class="flex-1 flex flex-col p-5">
                                ...
                                <div class="flex flex-wrap gap-1.5 mb-3">
                                    @for (tag of post.tags.slice(0, 2); track tag) {
                                        <span class="px-2 py-0.5 rounded-md bg-ochre-50 text-ochre-700 text-[11px] font-medium">
                                            {{ tag }}
                                        </span>
                                    }
                                </div>
                                ...
                                <!-- Title -->
                                <h2 class="text-lg font-bold text-surface-900 mb-2 leading-snug group-hover:text-brand-700 transition-colors">
                                    {{ post.title }}
                                </h2>

                                <!-- Excerpt -->
                                <p class="text-sm text-surface-600 leading-relaxed line-clamp-3 mb-4">
                                    {{ post.excerpt }}
                                </p>

                                <!-- Footer: date + read more -->
                                <div class="mt-auto flex items-center justify-between pt-3 border-t border-surface-100">
                                    <time class="text-xs text-surface-400">{{ formatDate(post.date) }}</time>
